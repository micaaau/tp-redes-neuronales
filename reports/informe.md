...
El análisis exploratorio fue clave para tomar decisiones de preprocesamiento justificadas. Permitió identificar que el dataset contenía valores faltantes en tres columnas (`ph`, `Sulfate` y `Trihalomethanes`), que se decidió imputar con la mediana en lugar de eliminar filas para no perder información útil. También permitió detectar outliers mediante el método IQR y decidir no eliminarlos, ya que se trataba de mediciones físico-químicas reales y no de errores. Por último, las diferencias de escala entre variables (algunas en miles, otras en unidades) llevaron a aplicar normalización Z-score, fundamental para que la red neuronal entrenara correctamente con descenso por gradiente.

El análisis de correlación también influyó directamente en el diseño de la red. Las correlaciones lineales entre las variables de entrada y `Potability` fueron muy bajas, lo que mostró que el problema no era lineal y justificó el uso de una red neuronal, capaz de capturar relaciones no lineales. Para la arquitectura se eligió una capa oculta con 6 neuronas, ReLU en la capa oculta para evitar el gradiente desvaneciente, y sigmoide en la salida por tratarse de clasificación binaria. La elección de 6 neuronas se confirmó luego en la Parte 3, donde las arquitecturas más grandes presentaron mayor sobreajuste.

La implementación manual con `numpy` permitió consolidar los conceptos fundamentales del funcionamiento de una red neuronal: la inicialización de pesos y sesgos, la propagación hacia adelante con multiplicación de matrices y funciones de activación, el cálculo de la función de costo, la retropropagación aplicando la regla de la cadena, y la actualización de parámetros mediante descenso por gradiente estocástico. Programar cada paso permitió entender cómo la red ajusta sus pesos a partir del error y no solo usar la red como una caja negra.

La comparación con `scikit-learn` mostró que ambas implementaciones obtuvieron rendimientos similares (alrededor del 65-68% de accuracy en prueba), lo que confirma que la red implementada manualmente funcionó correctamente. La librería resultó útil para probar rápidamente diferentes arquitecturas, mientras que la implementación manual fue más valiosa desde lo conceptual.

Finalmente, el rendimiento moderado del modelo refleja una limitación del dataset y no del enfoque utilizado: las variables disponibles no separan claramente las clases potable y no potable. Aumentar la complejidad de la red en la Parte 3 no mejoró la accuracy de prueba sino que la empeoró, lo que confirma que el límite está en los datos y no en la capacidad del modelo. En conclusión, el TP permitió integrar conceptos de análisis de datos, preprocesamiento, redes neuronales, entrenamiento y evaluación, y mostró que un buen análisis exploratorio es fundamental tanto para tomar decisiones justificadas como para interpretar correctamente los resultados.